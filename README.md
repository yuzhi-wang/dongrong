...
运行时显示完成进度、客户 ID、状态、Top1 / Top3 是否命中和耗时，结果逐条保存为 JSON。全部结束后，比较两轮模型原始排名：一项是与 Qwen3.7-Max 的93条全量基线比较，另一项在对应基线文件存在时，与上次 Qwen3.8-Max 的固定86位成功客户比较，用于观察同模型下的提示词变化。

随后自动生成一份 **HTML 报告**，集中展示数据来源和最终结果，终端打印 `HTML 报告：<完整路径>`。用浏览器打开该文件，即可查看命中率、历史比较和逐客户推荐详情。

这是一个生成报告后即退出的 Python 命令行项目，无需启动 Web 服务、数据库或 Docker。HTML 内嵌样式、脚本和数据，可离线打开；程序不会自动启动浏览器。
...
默认输出目录为 `outputs/validation_runs/`，文件关系如下：

| 文件或目录 | 内容 | 生成时机 |
| --- | --- | --- |
| `validatable_real_<时间戳>.html` | 面向阅读的完整报告 | 全部客户处理、结果保存和已启用的比较结束后 |
| `validatable_real_<时间戳>.json` | 运行配置、汇总和逐客户结果 | 初始化及每位客户处理完成后保存，结束时更新最终结果 |
| `validatable_real_<时间戳>_comparison.json` | 历史基线比较 | 启用比较时，在全部客户处理结束后生成 |
| `validatable_real_<时间戳>_raw/` | 收到的完整模型响应 | 每次收到完整响应后保存 |

`--output` 必须指定 `.json` 文件路径，HTML 自动使用同目录、同名的 `.html` 路径，无需另传参数。同一路径重跑会覆盖报告，程序会阻止覆盖正在使用的基线。`--dry-run` 默认文件名中的 `real` 改为 `dry_run`；`--no-compare` 仅关闭历史比较，仍生成 HTML。

### 如何阅读 HTML

双击终端打印路径对应的 `.html` 文件，用浏览器打开即可。报告包含：

- **最终结果**：本次客户数、完成推荐数、Top1、Recall@3、调用失败及硬规则排除数，同时展示仅成功客户的指标及其分母。
- **数据来源**：实际读取的客户与产品 JSON 路径、文件声明的上游来源、可验证客户总数、产品总数和提示词路径；可展开产品库快照与运行配置。
- **按真实产品统计**：每款真实产品对应的客户数、完成数、命中数和组内 Recall@3。
- **历史比较**：已启用的全量基线和同模型固定子集比较；不满足比较条件时展示具体原因。
- **逐客户最终结果**：支持搜索客户 ID、产品及理由，按完成、Top3 未命中、调用失败、硬规则排除、离线检查筛选；展开可查看 Top3 顺序、分数、置信度、推荐理由、待确认信息、客户输入、验证标签和完整结果记录。

Top1 表示首个推荐命中真实产品，Recall@3 表示前三个推荐包含真实产品。首页指标以本次全部客户为分母，硬规则排除和调用失败计为未命中；离线运行将模型指标标为“未评估”，不将未调用模型展示为零命中率。

报告内嵌本次客户与产品输入快照，单独保存 HTML 仍可阅读。来源路径描述本次运行所用文件，原始结果及比较 JSON 链接在对应文件仍存在时可用。结果 JSON 的 `run.data_sources` 同时记录来源信息。HTML 只在运行结束时生成，不会随进度实时刷新；中途退出可检查已保存的 JSON 快照。
...
只有主线程写文件，每完成一位客户保存一次完整 JSON 快照。终端按完成先后显示进度，JSON 中的客户始终按输入顺序排列。JSON 保存时遇到 Windows 短暂文件占用会有限重试原子替换；持续权限错误仍会报错并保留上一份完整报告。当前不支持自动断点续跑。
...
离线检查处理全部 93 位客户，只验证本地数据、硬过滤和报告生成；JSON 中模型命中率为 `null`，HTML 中显示“未评估”，已启用的比较报告为 `not_comparable`。如只想预览展示效果，可运行：

```powershell
& $PythonExe -X utf8 main.py --dry-run --no-compare --output outputs/validation_runs/html_preview.json
```

完成后打开 `outputs/validation_runs/html_preview.html`。该方式不依赖历史比较基线，也不读取 Key 或调用模型。
...
## HTML 报告验证记录（2026-09-07）

- 已执行全量 93 位客户的离线运行，生成 HTML、结果 JSON 和历史比较 JSON；91 位真实产品通过硬过滤，2 位被排除。未调用付费模型。
- 全套 48 项测试中 46 项通过、2 项报错。报错原因是本机缺少 `outputs/validation_runs/validatable_real_20260906_185128.json`，导致既有固定子集测试无法读取基线、全量模拟测试缺少预期的 `prompt_comparison` 字段。
- 新增的 2 项 HTML 测试通过，覆盖离线生成同名报告、来源与客户结果展示、失败原因、指标分母及模型文本 HTML 转义；随后补充可选字段空值兼容后，这 2 项再次通过。
- 浏览器安全策略阻止了本地 HTML 预览，尚未完成浏览器中的视觉和交互核验；以上验证不代表已核验页面布局或筛选交互。
...
| `html_report.py` | 运行结束后生成独立 HTML 报告，展示来源与最终结果 |
